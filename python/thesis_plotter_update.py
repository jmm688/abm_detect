...
def get_gtif_mines():   # takes all csv in a directory and will stack all mine related pixels into one dataset to be exported
  ...
  if os.path.isfile(ref_path):
      ...
      template = np.zeros([all_parts.shape[0],all_parts.shape[1],all_parts.shape[2]]) # CHHAHAHAHAANGE!!!!!!!!!!!!!!!
      template[:] = np.nan
      
      count = 0
      for i in range(0, all_parts.shape[0]):
        mines = np.where(ref_template == 1, all_parts[count,:,:].flatten(), ref_template)
        mines = mines.reshape([1,400,400])
        
        template[count,:,:] = mines[0,:,:]
        #count+=1
        
        #sns.heatmap(template[count,:,:])
        #plt.show()
        count+=1
        #break
